...
// Client-side validation can be bypassed (devtools, curl, replay), so the
// server is the real gate. Enforce every field the form marks required, plus
// the legal agreement — otherwise a partial/forged payload would be accepted.
function requiredFieldsFor(isBusiness) {
  const legal = [
    ['Signature', 'Missing electronic signature'],
    ['LegalAgreed', 'Authorization agreement is required'],
  ...
  return isBusiness
    ? [
        ...
        ['PersonalGuarantorSSN', 'Missing guarantor SSN'],
        ['GrossAnnualIncome', 'Missing gross annual income'],
        ...legal,
      ]
    : [
        ...
        ['SSN', 'Missing Social Security Number'],
        ['DOB', 'Missing date of birth'],
        ['Street', 'Missing street address'],
        ['City', 'Missing city'],
        ['State', 'Missing state'],
...
/**
 * Validate a credit-application payload against the required-field rules.
 * Exported so callers can reject (and avoid persisting) junk before processing.
 * @param {Record<string, any>} body
 * @returns {{ valid: boolean, error?: string }}
 */
export function validateCreditApplication(body) {
...
export async function processCreditApplication(body) {
  ...
  if (!valid) {
    return { status: 400, json: { success: false, error } };
  }
