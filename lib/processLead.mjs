...
import { escapeHtml, labelKey, resolveConsultantRecipients, CONTACT_CC_EMAILS, TEAM_CC_EMAILS } from './sendEmailSecurity.mjs';
...
export async function processLead(type, body) {
  ...
  // Contact Us: Christopher/Henry/Kai only (no sales@). Trade-in: full team CC, unchanged.
  const teamCc = type === 'contact' ? CONTACT_CC_EMAILS : TEAM_CC_EMAILS;
  const toSet = new Set(to.map((e) => e.toLowerCase()));
  const cc = override ? [] : teamCc.filter((e) => !toSet.has(e.toLowerCase()));
