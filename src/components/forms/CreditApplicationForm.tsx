    ...
    if (currentStep === 1) {
      ...
      if (values.coApplicantEnabled) {
        ...
        if (email && !z.string().email().safeParse(email).success) {
          setError('coEmail', { message: 'Please enter a valid email address', type: 'manual' });
        }
        ...
        if (!isCoApplicantSectionValid(values)) {
          ...
          if (!values.coFirstName?.trim()) {
            setError('coFirstName', { message: 'First name is required', type: 'manual' });
          }
          ...
          if (!values.coHousing?.trim()) {
            setError('coHousing', { message: 'Housing status is required', type: 'manual' });
          }
          if (!values.coPhone?.trim() || (values.coPhone?.trim()?.length ?? 0) < 10) {
            setError('coPhone', { message: 'Valid phone number is required', type: 'manual' });
          }
          if (!email) {
            setError('coEmail', { message: 'Email is required', type: 'manual' });
          }
          if (!values.coStreet?.trim()) {
            setError('coStreet', { message: 'Street is required', type: 'manual' });
          }
          if (!values.coCity?.trim()) {
            setError('coCity', { message: 'City is required', type: 'manual' });
          }
          if (!values.coState?.trim()) {
            setError('coState', { message: 'State is required', type: 'manual' });
          }
          if (!values.coZip?.trim() || (values.coZip?.trim()?.length ?? 0) < 5) {
            setError('coZip', { message: 'ZIP is required', type: 'manual' });
          }
          if (!values.coYearsAtResidence?.trim()) {
            setError('coYearsAtResidence', { message: 'Years at residence is required', type: 'manual' });
          }
          if (!values.coMonthlyPayment?.trim()) {
            setError('coMonthlyPayment', { message: 'Monthly payment is required', type: 'manual' });
          }
          if (!values.coEmployer?.trim()) {
            setError('coEmployer', { message: 'Employer is required', type: 'manual' });
          }
          if (!values.coPosition?.trim()) {
            setError('coPosition', { message: 'Position is required', type: 'manual' });
          }
          if (!values.coGrossAnnualIncome?.trim()) {
            setError('coGrossAnnualIncome', { message: 'Gross annual income is required', type: 'manual' });
          }
      ...
      if (currentStep === 1) {
        ...
        if (values.coApplicantEnabled) {
          ...
          if (!isCoApplicantSectionValid(values)) {
            ...
            if (!email) setError('coEmail', { message: 'Email is required', type: 'manual' });
            if (!values.coStreet?.trim()) setError('coStreet', { message: 'Street is required', type: 'manual' });
            if (!values.coCity?.trim()) setError('coCity', { message: 'City is required', type: 'manual' });
            if (!values.coState?.trim()) setError('coState', { message: 'State is required', type: 'manual' });
            if (!values.coZip?.trim() || (values.coZip?.trim()?.length ?? 0) < 5) setError('coZip', { message: 'ZIP is required', type: 'manual' });
            if (!values.coYearsAtResidence?.trim()) setError('coYearsAtResidence', { message: 'Years at residence is required', type: 'manual' });
            if (!values.coMonthlyPayment?.trim()) setError('coMonthlyPayment', { message: 'Monthly payment is required', type: 'manual' });
            if (!values.coEmployer?.trim()) setError('coEmployer', { message: 'Employer is required', type: 'manual' });
            if (!values.coPosition?.trim()) setError('coPosition', { message: 'Position is required', type: 'manual' });
            if (!values.coGrossAnnualIncome?.trim()) setError('coGrossAnnualIncome', { message: 'Gross annual income is required', type: 'manual' });
      ...
      <AnimatePresence mode="wait" custom={direction}>
        ...
        {currentStep === 1 && (
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground dark:text-white/70">Let's start with your basic information</p>
          </div>
          <div className="grid md:grid-cols-2 gap-5">
            <div className="space-y-1.5">
              <Label htmlFor="firstName" className="text-sm font-medium flex items-center gap-1.5">
                First Name <span className="text-destructive">*</span>
                {isFieldValid('firstName') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
              </Label>
              ...
              {errors.firstName && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.firstName.message}</p>}
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="lastName" className="text-sm font-medium flex items-center gap-1.5">
                Last Name <span className="text-destructive">*</span>
                {isFieldValid('lastName') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
              </Label>
              <div className="relative">
                <Input 
                  id="lastName" 
              ...
              {errors.lastName && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.lastName.message}</p>}
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="ssn" className="text-sm font-medium flex items-center gap-1.5">
                <Lock className="w-3.5 h-3.5 text-muted-foreground" />
                Social Security Number <span className="text-destructive">*</span>
                {isFieldValid('ssn') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
              </Label>
              <div className="relative">
                <Input 
                  id="ssn" 
            ...
            <div className="space-y-1.5">
              <Label htmlFor="dob" className="text-sm font-medium flex items-center gap-1.5">
                ...
                {isFieldValid('dob') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
              </Label>
              <div className="relative">
                <Input 
                  id="dob" 
              ...
              {errors.dob && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.dob.message}</p>}
            </div>
            <div className="space-y-1.5 md:col-span-2">
              <Label className="text-sm font-medium flex items-center gap-1.5">
                <Home className="w-3.5 h-3.5 text-muted-foreground" />
                Current Housing Status <span className="text-destructive">*</span>
                {isFieldValid('housing') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
              </Label>
              <Select value={watch('housing') ?? ''} onValueChange={async (v) => {
                setValue('housing', v, { shouldValidate: true, shouldTouch: true });
              ...
              {errors.housing && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.housing.message}</p>}
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-border dark:border-white/10">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="phone" className="text-sm font-medium flex items-center gap-1.5">
                ...
                {isFieldValid('phone') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
              </Label>
              <div className="relative">
                <Input 
                  id="phone" 
          ...
          <div className="pt-4 border-t border-border dark:border-white/10">
            ...
            <div className="grid md:grid-cols-2 gap-5">
              <div className="space-y-1.5 md:col-span-2">
                ...
                {errors.street && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.street.message}</p>}
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="city" className="text-sm font-medium flex items-center gap-1.5">
                  City <span className="text-destructive">*</span>
                  {isFieldValid('city') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                </Label>
                <div className="relative">
                  <Input 
                    id="city" 
                ...
                {errors.city && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.city.message}</p>}
              </div>
            <div className="space-y-1.5">
              <Label className="text-sm font-medium flex items-center gap-1.5">
                State <span className="text-destructive">*</span>
                {isFieldValid('state') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
              </Label>
              <Select value={watch('state') ?? ''} onValueChange={async (v) => {
                setValue('state', v, { shouldValidate: true, shouldTouch: true });
                await trigger('state');
              }}>
                <SelectTrigger className={cn(
                  errors.state 
              ...
              {errors.state && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.state.message}</p>}
            </div>
              <div className="space-y-1.5">
                <Label htmlFor="zip" className="text-sm font-medium flex items-center gap-1.5">
                  ZIP Code <span className="text-destructive">*</span>
                  {isFieldValid('zip') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                </Label>
                <div className="relative">
                  <Input 
                    id="zip" 
                ...
                {errors.zip && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.zip.message}</p>}
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="yearsAtResidence" className="text-sm font-medium flex items-center gap-1.5">
                  Years at This Address <span className="text-destructive">*</span>
                  {isFieldValid('yearsAtResidence') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                </Label>
                <div className="relative">
                  <Input 
                    id="yearsAtResidence" 
                  ...
                  />
                  {isFieldValid('yearsAtResidence') && !errors.yearsAtResidence && (
                    <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-green-500 pointer-events-none" />
                  )}
                </div>
                <p className="text-xs text-muted-foreground dark:text-white/60">How long have you lived here?</p>
                {errors.yearsAtResidence && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.yearsAtResidence.message}</p>}
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="monthlyPayment" className="text-sm font-medium flex items-center gap-1.5">
                  <DollarSign className="w-3.5 h-3.5 text-muted-foreground" />
                  Monthly Housing Payment <span className="text-destructive">*</span>
                  {isFieldValid('monthlyPayment') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                </Label>
                <div className="relative">
                  <Input 
                    id="monthlyPayment" 
                  ...
                  />
                  {isFieldValid('monthlyPayment') && !errors.monthlyPayment && (
                    <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-green-500 pointer-events-none" />
                  )}
                </div>
        ...
        >
          <div>
            ...
            <p className="text-sm text-muted-foreground dark:text-white/70">Tell us about your current employment</p>
          </div>
          <div className="grid md:grid-cols-2 gap-5">
            <div className="space-y-1.5 md:col-span-2">
              <Label htmlFor="employer" className="text-sm font-medium flex items-center gap-1.5">
                Company Name <span className="text-destructive">*</span>
                {isFieldValid('employer') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
              </Label>
              <div className="relative">
                <Input 
                  id="employer" 
            ...
            <div className="space-y-1.5">
              <Label htmlFor="position" className="text-sm font-medium flex items-center gap-1.5">
                ...
                {isFieldValid('position') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
              </Label>
              <div className="relative">
                <Input 
                  id="position" 
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                />
                {isFieldValid('grossAnnualIncome') && !errors.grossAnnualIncome && (
                  <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-green-500 pointer-events-none" />
                )}
              </div>
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground dark:text-white/60">Additional income from investments, rental, etc.</p>
            </div>
            <div className="space-y-1.5 md:col-span-2">
              <Label htmlFor="organizationAffiliation" className="text-sm font-medium">Organization Affiliation</Label>
              <Input id="organizationAffiliation" {...register('organizationAffiliation')} placeholder="e.g. Professional Association, Union, etc." />
              <p className="text-xs text-muted-foreground dark:text-white/60">Any professional organizations or memberships (optional)</p>
            </div>
          </div>
        </motion.div>
      )}
        ...
        >
          <div>
            ...
            <p className="text-sm text-muted-foreground dark:text-white/70">Add a co-applicant to strengthen your application (optional)</p>
          </div>
          <div className="flex items-start gap-3 p-4 rounded-xl border-2 border-border dark:border-white/10 bg-muted/30 dark:bg-white/[0.02]">
            ...
            <div className="flex-1">
              <Label htmlFor="coApplicantEnabled" className="text-sm font-medium cursor-pointer flex items-center gap-2">
                I would like to add a co-applicant
              </Label>
              <p className="text-xs text-muted-foreground dark:text-white/60 mt-1">A co-applicant can help improve your chances of approval</p>
            </div>
          </div>
          ...
          {coApplicantEnabled && (
            <div className="rounded-xl border border-border dark:border-white/10 bg-muted/50 dark:bg-white/[0.02] p-6 space-y-6">
              <p className="text-sm text-foreground dark:text-white font-medium">Please provide the same information for your co-applicant</p>
                ...
                <div>
                  ...
                  <div className="grid md:grid-cols-2 gap-5">
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        First Name <span className="text-destructive">*</span>
                        {isFieldValid('coFirstName') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                      </Label>
                      <div className="relative">
                        <Input 
                          id="coFirstName"
                        ...
                        />
                        {isFieldValid('coFirstName') && !errors.coFirstName && (
                          <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-green-500 pointer-events-none" />
                        )}
                      </div>
                      {errors.coFirstName && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.coFirstName.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        Last Name <span className="text-destructive">*</span>
                        {isFieldValid('coLastName') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                      </Label>
                      <div className="relative">
                        <Input 
                          {...register('coLastName', {
                      ...
                      {errors.coLastName && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.coLastName.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        <Lock className="w-3.5 h-3.5 text-muted-foreground" />
                        Social Security Number <span className="text-destructive">*</span>
                        {isFieldValid('coSsn') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                      </Label>
                      <div className="relative">
                        <Input 
                          id="coSsn"
                      ...
                      {errors.coSsn && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.coSsn.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        <Calendar className="w-3.5 h-3.5 text-muted-foreground" />
                        Date of Birth <span className="text-destructive">*</span>
                        {isFieldValid('coDob') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                      </Label>
                      <div className="relative">
                        <Input 
                          id="coDob"
                      ...
                      {errors.coDob && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.coDob.message}</p>}
                    </div>
                    <div className="space-y-1.5 md:col-span-2">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        <Home className="w-3.5 h-3.5 text-muted-foreground" />
                        Current Housing Status <span className="text-destructive">*</span>
                        {isFieldValid('coHousing') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                      </Label>
                      <Select value={watch('coHousing') ?? ''} onValueChange={async (v) => {
                        setValue('coHousing', v, { shouldValidate: true, shouldTouch: true });
                      ...
                      {errors.coHousing && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.coHousing.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        Phone Number <span className="text-destructive">*</span>
                        {isFieldValid('coPhone') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                      </Label>
                      <div className="relative">
                        <Input 
                          id="coPhone"
                      ...
                      {errors.coPhone && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.coPhone.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        <Mail className="w-3.5 h-3.5 text-muted-foreground" />
                        Email Address <span className="text-destructive">*</span>
                        {isFieldValid('coEmail') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                      </Label>
                      <div className="relative">
                        <Input 
                          id="coEmail"
                      ...
                      {errors.coEmail && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.coEmail.message}</p>}
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-border dark:border-white/10">
                  ...
                  <div className="grid md:grid-cols-2 gap-5">
                    <div className="space-y-1.5 md:col-span-2">
                      ...
                      {errors.coStreet && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.coStreet.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        City <span className="text-destructive">*</span>
                        {isFieldValid('coCity') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                      </Label>
                      <div className="relative">
                        <Input 
                          {...register('coCity', {
                      ...
                      {errors.coCity && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.coCity.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        State <span className="text-destructive">*</span>
                        {isFieldValid('coState') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                      </Label>
                      <Select value={watch('coState') ?? ''} onValueChange={async (v) => {
                        setValue('coState', v, { shouldValidate: true, shouldTouch: true });
                      ...
                      {errors.coState && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.coState.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        ZIP Code <span className="text-destructive">*</span>
                        {isFieldValid('coZip') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                      </Label>
                      <div className="relative">
                        <Input 
                          id="coZip"
                      ...
                      {errors.coZip && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.coZip.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        Years at This Address <span className="text-destructive">*</span>
                        {isFieldValid('coYearsAtResidence') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                      </Label>
                      <div className="relative">
                        <Input 
                        ...
                        />
                        {isFieldValid('coYearsAtResidence') && !errors.coYearsAtResidence && (
                          <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-green-500 pointer-events-none" />
                        )}
                      </div>
                      {errors.coYearsAtResidence && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.coYearsAtResidence.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        <DollarSign className="w-3.5 h-3.5 text-muted-foreground" />
                        Monthly Housing Payment <span className="text-destructive">*</span>
                        {isFieldValid('coMonthlyPayment') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                      </Label>
                      <div className="relative">
                        <Input 
                          id="coMonthlyPayment"
                        ...
                        />
                        {isFieldValid('coMonthlyPayment') && !errors.coMonthlyPayment && (
                          <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-green-500 pointer-events-none" />
                        )}
                      </div>
                      {errors.coMonthlyPayment && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.coMonthlyPayment.message}</p>}
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-border dark:border-white/10">
                  ...
                  <div className="grid md:grid-cols-2 gap-5">
                    <div className="space-y-1.5 md:col-span-2">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        Company Name <span className="text-destructive">*</span>
                        {isFieldValid('coEmployer') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                      </Label>
                      <div className="relative">
                        <Input 
                          {...register('coEmployer', {
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        Job Title / Position <span className="text-destructive">*</span>
                        {isFieldValid('coPosition') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                      </Label>
                      <div className="relative">
                        <Input 
                          id="coPosition"
                      ...
                      {errors.coPosition && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.coPosition.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        <DollarSign className="w-3.5 h-3.5 text-muted-foreground" />
                        Gross Annual Income <span className="text-destructive">*</span>
                        {isFieldValid('coGrossAnnualIncome') && <CheckCircle2 className="w-3.5 h-3.5 text-green-500" />}
                      </Label>
                      <div className="relative">
                        <Input 
                        ...
                        />
                        {isFieldValid('coGrossAnnualIncome') && !errors.coGrossAnnualIncome && (
                          <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-green-500 pointer-events-none" />
                        )}
                      </div>
                      {errors.coGrossAnnualIncome && <p className="text-xs text-destructive mt-1 flex items-center gap-1"><X className="w-3 h-3" /> {errors.coGrossAnnualIncome.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium flex items-center gap-1.5">
                        <DollarSign className="w-3.5 h-3.5 text-muted-foreground" />
                      ...
                      <Input {...register('coOtherAnnualIncome')} placeholder="e.g. 5000" />
                    </div>
                    <div className="space-y-1.5 md:col-span-2">
                      <Label className="text-sm font-medium">Organization Affiliation</Label>
                      <Input {...register('coOrganizationAffiliation')} placeholder="e.g. Professional Association, Union, etc." />
                    </div>
                  </div>
                </div>
              </div>
        ...
        >
          <div>
            ...
            <p className="text-sm text-muted-foreground dark:text-white/70">Upload any supporting documents to help us process your application faster (optional)</p>
          </div>
          <div className="grid md:grid-cols-2 gap-5">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground dark:text-white/60">e.g. Pay stubs, bank statements, ID</p>
            </div>
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground dark:text-white/60">e.g. Tax returns, proof of income</p>
            </div>
          </div>
...
errors.consultant 
            ...
            <p className="text-xs text-muted-foreground dark:text-white/60">Who would you like to work with?</p>
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground dark:text-white/60">This serves as your electronic signature</p>
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground dark:text-white/60">Today&apos;s date will be automatically recorded when you submit</p>
          </div>
        </motion.div>
      )}
      </AnimatePresence>
